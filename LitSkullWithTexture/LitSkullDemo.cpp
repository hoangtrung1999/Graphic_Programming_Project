...
private:
	...
	ID3D11ShaderResourceView* mColumnDiffuseMapSRV;
	ID3D11ShaderResourceView* mSphereDiffuseMapSRV;
	ID3D11ShaderResourceView* mBoxDiffuseMapSRV;
	ID3D11ShaderResourceView* mPlaneDiffuseMapSRV;
  ...
  mEyePosW(0.0f, 0.0f, 0.0f), mTheta(1.5f*MathHelper::Pi), mPhi(0.1f*MathHelper::Pi), mRadius(15.0f)
{
	...
	mSkullMat.Specular = XMFLOAT4(0.6f, 0.6f, 0.6f, 16.0f);
}
...
LitSkullApp::~LitSkullApp()
{
	...
	ReleaseCOM (mColumnDiffuseMapSRV);
	ReleaseCOM (mSphereDiffuseMapSRV);
	ReleaseCOM (mPlaneDiffuseMapSRV);
	ReleaseCOM (mBoxDiffuseMapSRV);
...
bool LitSkullApp::Init()
{
	...
	// Create Shader Resource View for each obj
	HR(D3DX11CreateShaderResourceViewFromFile(md3dDevice, L"Textures/darkbrick.bmp", 0, 0, &mColumnDiffuseMapSRV, 0));
	HR(D3DX11CreateShaderResourceViewFromFile(md3dDevice, L"Textures/flare.dds", 0, 0, &mSphereDiffuseMapSRV, 0));
	HR(D3DX11CreateShaderResourceViewFromFile(md3dDevice, L"Textures/flarealpha.dds", 0, 0, &mPlaneDiffuseMapSRV, 0));
	HR(D3DX11CreateShaderResourceViewFromFile(md3dDevice, L"Textures/WoodCrate01.dds", 0, 0, &mBoxDiffuseMapSRV, 0));
...
void LitSkullApp::DrawScene()
{
    ...
    for(UINT p = 0; p < techDesc.Passes; ++p)
    {
		...
		Effects::BasicFX->SetDiffuseMap(mPlaneDiffuseMapSRV);
		...
		Effects::BasicFX->SetDiffuseMap(mBoxDiffuseMapSRV);
		...
		for(int i = 0; i < 10; ++i)
		{
			...
			Effects::BasicFX->SetDiffuseMap(mColumnDiffuseMapSRV);
		...
		for(int i = 0; i < 10; ++i)
		{
			...
			Effects::BasicFX->SetDiffuseMap(mSphereDiffuseMapSRV);
